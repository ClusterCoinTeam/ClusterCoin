...
#if defined(NDEBUG)
# error "ClusterCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ClusterCoin Signed Message:\n";
...
int64_t GetBlockValue(int nHeight, int64_t nFees)
{
    ...
    // 13,001 - 10,000,000 blocks: 3 CLSTR
    // 10,000,001 - .. : 0 CLSTR
    ...
    } else if (nHeight > 13000 && nHeight <= 10000000)
    {
      nSubsidy = 3 * COIN;
    }
...
void ThreadScriptCheck() {
    RenameThread("clustercoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        else
        {
            ...
            const int64_t nTxValueIn = view.GetValueIn(tx);
            const int64_t nTxValueOut = tx.GetValueOut();
            nValueIn += nTxValueIn;
    ...
    if (!fJustCheck)
    {
        // ClusterCoin: track money supply
...
#if 0 // disable prerelease warning
